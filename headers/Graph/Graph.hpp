...
public:
	...
	int load_from_file(std::string fileName);
  	...
  	std::list<Vertex> dijkstra(unsigned int idSource, unsigned int idDestination);
...
private:
	...
	std::list<Vertex> dijkstra_get_path(const unsigned int idSource, const unsigned int idDestination, const unsigned int *pere);
