...
bool ButtonTexture::button_pressed(sf::Window& window, const sf::Event& event)
{
...
bool ButtonTexture::button_activated(sf::Window& window, const sf::Event& event)
{
	if(sf::Mouse::getPosition(window).x >= this->getPosition().x &&
		sf::Mouse::getPosition(window).x <= this->getPosition().x + dimension.x &&
		sf::Mouse::getPosition(window).y >= this->getPosition().y &&
...
{
	texture_idle.loadFromFile(path);
